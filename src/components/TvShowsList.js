...
function TvShowList() {      
  ...
  const {data, error, isFetching } = useFetchTvShowsQuery(selectedGenre);    //kaldet vil straks hente data i et result-objekt, som vi "destructure" til data, error og isLoading
...
let content;
  if (isFetching) {
    ...
    content = data.results.filter(tv => tv.poster_path !== null).map((tv) => {
      return <TvShowCard key={tv.id} movie={tv}></TvShowCard>
    });
  }
    return (
